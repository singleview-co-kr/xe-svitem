 ...
 */
function addItemsToFavoritesInCatalogPage(nItemSrl)
{
	var params = new Array();
	// if item_srl is not passed, throw item_srl list delimited with comma.
	if (typeof(nItemSrl)=='undefined')
		params['item_srl'] = _makeItemList();
	else
...
//console.log( params);
	exec_xml('svitem', 'procSvitemAddItemsToFavorites', params, function(ret_obj) {
		...
		if (confirm('관심상품에 추가하였습니다. 관심상품으로 이동하시겠습니까?')) 
		{
			...
			location.href = current_url.setQuery('mid', g_sSvcartMid).setQuery('act','dispSvcartFavoriteItems');
		}
	});
}
//////////////////////////////////////////////
...
function wishlist_nc( sUrl ) 
{
	...
	{
		window.open(sUrl,'','scrollbars=yes,width=400,height=267');
		return false;
	}
}
/**
